...
<%@ attribute name="showPageSize" required="false" rtexprvalue="true"
	description="Show record per page option"%>

<%@ attribute name="recordPageTitle" required="false" rtexprvalue="true"
	description="Title for record per page option"%>

<c:set var="pageNumber" value="${thispage.number}" />
<c:set var="maxPages"
	value="${thispage.totalPages > 0 ? thispage.totalPages: 5}" />
...
<c:set var="showPageSize"
	value="${showPageSize != null ? showPageSize : true}" />
<c:set var="pageSizeTitle"
	value="${recordPageTitle != null ? recordPageTitle : 'Record per page'}" />
		...
		<c:choose>
			<c:when test="${pageNumber ne 0}">
				...
				<li class="page-item"><a class="page-link" href="${first}"
					title="${fn:escapeXml(first_label)}"><span aria-hidden="true">&laquo;</span></a></li>
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${pageNumber gt 0}">
				...
				<li class="page-item"><a class="page-link" href="${previous}"
					title="${fn:escapeXml(previous_label)}"><span
		...
		<c:choose>
			<c:when test="${pageNumber lt (maxPages-1)}">
				...
				<li class="page-item"><a class="page-link" href="${next}"
					title="${fn:escapeXml(next_label)}"><span aria-hidden="true">&#8250;</span></a></li>
			</c:when>
			<c:otherwise>
				<li class="page-item disabled"><a class="page-link" href="#"><span
						aria-hidden="true">&#8250;</span></a></li>
