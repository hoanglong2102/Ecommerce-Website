		...
		<c:when test="${ketqua}">
			...
			<div class="table-responsive">
				<table class="table table-striped table-sm">
					...
					<tbody>
						<c:forEach items="${danhmucs.getContent()}" var="danhmuc">
							<tr>
								<td><a
			...
			<c:if test="${danhmucs.totalPages > 0}">
				<util:pagination thispage="${danhmucs}"
					recordPageTitle="${categoryPage}" currentPage="${currentPage}">
				</util:pagination>
			</c:if>
		</c:when>
		<c:otherwise>
			<h4>
				<spring:message code="category.error"></spring:message>
			</h4>
		</c:otherwise>
	</c:choose>
